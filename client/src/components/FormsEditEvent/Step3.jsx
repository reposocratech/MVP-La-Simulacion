...
const Step3 = () => {
  ...
  useEffect(() => {
    if (dataTotal.sections?.length > 0) {
      setDataTotal(prev => ({
        ...prev,
        section_public: {
          ...prev.section_public,
          key_points: [
            ...dataTotal.sections[0].section_key_points // lo que viene de DB
          ]
        }
      }));
    }
        ...
        <Row className="justify-content-between gy-4">
          ...
          <Col lg={7}>
            {dataTotal?.section_public?.key_points.map((elem, index) => (
              <div className="d-flex justify-content-between gap-3 mb-3 bg-light rounded-4 p-2"  key={elem.pto_id ?? index}>
                <div>

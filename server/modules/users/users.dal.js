...
class UserDal {
  ...
  findUserByEmailLogin = async (email) => {
    try {
      let sql =
        'SELECT user_id, password FROM user WHERE email = ? AND user_is_deleted = 0 AND user_is_confirmed = 1 AND user_is_disabled = 0'
  ...
  userById = async (id) => {
    try {
      let sql =
        'SELECT * FROM user WHERE user_id = ? AND user_is_deleted = 0 AND user_is_disabled = 0 AND user_is_confirmed = 1'
  ...
  editUser = async (data) => {
    try {
      const { user_name, lastname, phone_number, specialty, user_id, avatar } =
        data
      let sql =
        'UPDATE user SET user_name = ?, lastname = ?, phone_number = ?, specialty = ? WHERE user_id = ?'
      let values = [user_name, lastname, phone_number, specialty, user_id]
      if (avatar) {
        sql =
          'UPDATE user SET user_name = ?, lastname = ?, phone_number = ?, specialty = ?, avatar = ? WHERE user_id = ?'
